...
function generateSquarePath(cx: number, cy: number, size: number): string {
  const h = size / 2;
  return `M ${cx - h} ${cy - h} L ${cx + h} ${cy - h} L ${cx + h} ${cy + h} L ${
    cx - h
  } ${cy + h} Z`;
}
...
function generateRectanglePath(
  ...
  const hw = w / 2;
  const hh = h / 2;
  return `M ${cx - hw} ${cy - hh} L ${cx + hw} ${cy - hh} L ${cx + hw} ${
    cy + hh
  } L ${cx - hw} ${cy + hh} Z`;
}
...
function generateTrianglePath(cx: number, cy: number, size: number): string {
  ...
  const hs = size / 2;
  return `M ${cx} ${cy - height / 1.5} L ${cx + hs} ${cy + height / 3} L ${
    cx - hs
  } ${cy + height / 3} Z`;
}
...
function generateHeartPath(cx: number, cy: number, size: number): string {
  const s = size / 30;
  return `M ${cx} ${cy + 12 * s} C ${cx - 20 * s} ${cy - 5 * s}, ${
    cx - 12 * s
  } ${cy - 18 * s}, ${cx} ${cy - 8 * s} C ${cx + 12 * s} ${cy - 18 * s}, ${
    cx + 20 * s
  } ${cy - 5 * s}, ${cx} ${cy + 12 * s}`;
}
...
function generateInfinityPath(
  ...
  const hw = w / 2;
  const hh = h / 2;
  return `M ${cx} ${cy} C ${cx + hw * 0.5} ${cy - hh}, ${cx + hw} ${cy - hh}, ${
    cx + hw
  } ${cy} C ${cx + hw} ${cy + hh}, ${cx + hw * 0.5} ${cy + hh}, ${cx} ${cy} C ${
    cx - hw * 0.5
  } ${cy + hh}, ${cx - hw} ${cy + hh}, ${cx - hw} ${cy} C ${cx - hw} ${
    cy - hh
  }, ${cx - hw * 0.5} ${cy - hh}, ${cx} ${cy}`;
}
...
export default function OrbitImages({
  ...
  useEffect(() => {
    if (paused) return;
    const target = direction === "reverse" ? -100 : 100;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const controls = animate(progress, target as any, {
      duration,
